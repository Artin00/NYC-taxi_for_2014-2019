...
vendor_dfr = vendor_df.withColumn("svendor_uuids", generated_uuid_udf())
vendor_dfr.display()
vendor_dfr.write.format("delta").saveAsTable("vendor_dimension")
...
payment_type_dfse.write.format("delta").saveAsTable("payment_dimension")
...
df2st.show()
df2st.count()
df2st.write.format("delta").saveAsTable("time_dimension")
...
df30.write.format("delta").saveAsTable("date_dimension")
...
display(cram)
cram.write.format("delta").saveAsTable("location_dimension")

# COMMAND ----------

ven = spark.read.table("vendor_dimension")
display(ven)

pay = spark.read.table("payment_dimension")
display(pay)

loc = spark.read.table("location_dimension")
display(loc)

date = spark.read.table("date_dimension")
display(date)

time = spark.read.table("time_dimension")
display(time)

# COMMAND ----------

# Add the assigned key to the main table and remove pickup/dropoff time 
df2st_new = time.drop("hour", "minute", "second")
...
df30_new = date.drop("year", "month", "day")
...
silver_fact_table = silver_fact_dope.withColumnRenamed("payment_type", "payment_type_id") 
silver_fact_tables_is = silver_fact_table.select("vendor_id", "pickup_date_id", "pickup_time_id", "dropoff_date_id", "dropoff_time_id", "passenger_count", "trip_distance", "pickup_location_id", "dropoff_location_id", "payment_type_id", "fare_amount", "extra", "mta_tax", "tip_amount", "tolls_amount", "total_amount")

# COMMAND ----------

# Assign key to payment_type_id
payway = pay.drop("payment_type_text")
...
crane = loc.drop("location_name")

near_gold  =  silver_fact_tables_is_news.join(crane, silver_fact_tables_is_news.pickup_location_id == crane.location_id, "inner")
...
vend = ven.drop("vendor_name", "vendor_id")

noway = nearly_golden.join(vend, nearly_golden.vendor_id == vend.vendor_number, "inner")
display(noway.limit(30))
noaways = noway.drop("vendor_id", "vendor_number")
noaways_now = noaways.withColumnRenamed("svendor_uuids", "vendor_id")

# COMMAND ----------

gold_completed = noaways_now.withColumn("trip_id", generated_uuid_udf())

gold_complete_final = gold_completed.select("trip_id", "vendor_id", "pickup_date_id", "pickup_time_id", "dropoff_date_id", "dropoff_time_id", "passenger_count", "trip_distance", "pickup_location_id", "dropoff_location_id", "payment_types_id", "fare_amount", "extra", "mta_tax", "tip_amount", "tolls_amount", "total_amount")
gold_complete_final.printSchema()


# COMMAND ----------

# Creation of delta fact_tables within databricks for PowerBI use
gold_complete_final.write.format("delta").saveAsTable("taxi_fact_tab")
...
pay.write.format("delta").saveAsTable("pay_dimi", path = "abfss://gold-artin@datacohortworkspacelabs.dfs.core.windows.net/payment_dimi")

# COMMAND ----------

date.write.format("delta").saveAsTable("date_dimi", path = "abfss://gold-artin@datacohortworkspacelabs.dfs.core.windows.net/date_dim")

# COMMAND ----------

loc.write.format("delta").saveAsTable("loca_dimi", path = "abfss://gold-artin@datacohortworkspacelabs.dfs.core.windows.net/location_dim")

# COMMAND ----------

ven.write.format("delta").saveAsTable("ven_dimi", path = "abfss://gold-artin@datacohortworkspacelabs.dfs.core.windows.net/vendor_dim")

# COMMAND ----------

time.write.format("delta").saveAsTable("time_dimi", path = "abfss://gold-artin@datacohortworkspacelabs.dfs.core.windows.net/time_dim")

# COMMAND ----------

gold_completed.write.format("delta").partitionBy("vendor_id").saveAsTable("fa_ab_taxi", path = "abfss://gold-artin@datacohortworkspacelabs.dfs.core.windows.net/taxi_fact")
