...
vendor_dfr.display()
vendor_dfr.write.format("delta").saveAsTable("vend_dimension")
...
payment_type_dfse.write.format("delta").saveAsTable("paym_dimension")
...
df2st.show()
df2st.count()
df2st.write.format("delta").saveAsTable("ti_dimension")
...
df30.write.format("delta").saveAsTable("da_dimension")
...
display(cram)
cram.write.format("delta").saveAsTable("loca_dimension")

# COMMAND ----------

ven = spark.read.table("vend_dimension")
display(ven)

pay = spark.read.table("paym_dimension")
display(pay)

loc = spark.read.table("loca_dimension")
display(loc)

date = spark.read.table("da_dimension")
display(date)

time = spark.read.table("ti_dimension")
display(time)
...
gold_complete_final = gold_completed.select("trip_id", "vendor_id", "pickup_date_id", "pickup_time_id", "dropoff_date_id", "dropoff_time_id", "passenger_count", "trip_distance", "pickup_location_id", "dropoff_location_id", "payment_type_id", "fare_amount", "extra", "mta_tax", "tip_amount", "tolls_amount", "total_amount")
...
gold_complete_final.write.format("delta").saveAsTable("taxi_fact_tab_new")
